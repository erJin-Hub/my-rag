...
def load_documents_from_folder(folder: str) -> List[Document]:
    ...
    for file_path in folder_path.iterdir():
        if file_path.is_file() and file_path.suffix in {".txt", ".md"}:
            ...
            print(f"  [LOAD] {file_path.name} ({len(content)} 字)")
...
if os.path.exists(INDEX_PATH) and os.path.exists(DOCS_PATH):
    ...
    print(f"[INFO] 未找到索引，从 {DOCS_DIR} 加载文档...")
    ...
    print(f"[OK] 构建完成：{index.ntotal} 个文档 → {INDEX_PATH}")
...
for i, (score, idx) in enumerate(zip(scores[0], indices[0])):
    if idx >= 0:
        print(f"  [{i+1}] 分数: {score:.4f} | 来源: {documents[idx].metadata['source']} | {documents[idx].page_content[:50]}...")
